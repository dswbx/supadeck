...
describe("resolveThemeModulePath", () => {
   ...
   it("resolves a theme directory to index.tsx", async () => {
      const root = await fs.mkdtemp(
         path.join(os.tmpdir(), "supadeck-theme-dir-")
      );
      tempRoots.push(root);
      ...
      expect(
         resolveThemeModulePath(path.join(root, "deck.mdx"), "./themes/custom")
      ).toBe(path.join(themeDir, "index.tsx"));
   });

   it("resolves an explicit theme file path", async () => {
      const root = await fs.mkdtemp(
         path.join(os.tmpdir(), "supadeck-theme-file-")
      );
      tempRoots.push(root);
      ...
      expect(
         resolveThemeModulePath(path.join(root, "deck.mdx"), "./my-theme.tsx")
      ).toBe(themeFile);
   });
});
