...
import { createSupadeckViteConfig } from "../src/runtime/vite-config.js";
...
function createModule(
   overrides: Partial<ModuleNode> & Pick<ModuleNode, "url">
): ModuleNode {
   return {
      url: overrides.url,
      ...
      clientImportedModules:
         overrides.clientImportedModules ?? new Set<ModuleNode>(),
      ...
      _getModuleSetUnion:
         overrides._getModuleSetUnion as ModuleNode["_getModuleSetUnion"],
      _getModuleInfoUnion:
         overrides._getModuleInfoUnion as ModuleNode["_getModuleInfoUnion"],
      _getModuleObjectUnion:
         overrides._getModuleObjectUnion as ModuleNode["_getModuleObjectUnion"],
   } as ModuleNode;
}

async function createWorkspace() {
   const root = await fs.mkdtemp(
      path.join(os.tmpdir(), "supadeck-tailwind-hmr-")
   );
   tempRoots.push(root);
...
function getSupadeckPlugin(deckPath: string): Plugin {
   ...
   const plugin = (config.plugins ?? []).find(
      (candidate: unknown): candidate is Plugin =>
         ...
         candidate !== null &&
         "name" in candidate &&
         candidate.name === "supadeck"
   );

   if (!plugin?.handleHotUpdate) {
      throw new Error("Supadeck plugin is missing handleHotUpdate.");
   }
...
describe("tailwind HMR integration", () => {
   it("invalidates slide js and emits a css update for deck changes", async () => {
      ...
      const plugin = getSupadeckPlugin(deckPath);
   ...
   it("emits a css update for imported workspace component changes without hijacking js hmr", async () => {
      ...
      const harness = createServerHarness(
         new Map([[THEME_RESOLVED, themeModule]])
      );
